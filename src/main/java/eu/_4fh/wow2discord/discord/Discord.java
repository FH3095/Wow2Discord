...
public class Discord implements AutoCloseable {
    ...
    public Discord(EventListener... listeners) {
        ...
        ThreadPoolExecutor executor =
                new ThreadPoolExecutor(maximumThreads, maximumThreads, 90, TimeUnit.SECONDS, new LinkedBlockingQueue<>());
        executor.allowCoreThreadTimeOut(true);
        jda =
                JDABuilder.create(Config.discordToken, GatewayIntent.GUILD_MEMBERS, GatewayIntent.GUILD_PRESENCES, GatewayIntent.GUILD_MODERATION)
    ...
    private void updateCommands() {
        ...
        SubcommandData linkCmd =
                new SubcommandData("link", "Adds a link from a wow-character to a discord account").addOption(OptionType.INTEGER, "wow-char-id", "WOW character id", true, true)
        ...
        SubcommandData searchUnlinkedCmd =
                new SubcommandData("unlinked", "Search for not yet linked characters").addOption(OptionType.STRING, "char-name", "Pattern of character name", false)
        ...
        SubcommandData searchLinkedCmd =
                new SubcommandData("linked", "Search for already linked characters").addOption(OptionType.STRING, "char-name", "Pattern of character name", false)
                                                                                    .addOption(OptionType.INTEGER, "guild-rank", "Rank of the characters in the guild", false)
